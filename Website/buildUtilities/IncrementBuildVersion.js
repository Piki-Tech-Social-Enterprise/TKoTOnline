...
const buildVersion = process.env.REACT_APP_WEB_BUILD_VERSION;
...
if (!buildVersion) {
  console.error(chalk.red.bold(`'REACT_APP_WEB_BUILD_VERSION' for '${envName}' is '${chalk.gray.italic('undefined')}'`));
  ...
  fs.readFile(envCmdFileName, (error, content) => {
    ...
    currentEnv.REACT_APP_WEB_BUILD_VERSION = incrementedBuildVersion;
    ...
    fs.writeFile(envCmdFileName, envCmdFileAsString, error => {
      ...
      console.log(chalk.green.bold(`New build version for '${chalk.blue.italic(envName)}': ${chalk.yellow.italic(envCmdFileAsJson[envName].REACT_APP_WEB_BUILD_VERSION)}`));
    });
  });
}